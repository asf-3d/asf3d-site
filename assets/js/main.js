(() => {
  ...
  function whatsappUrl(message) {
    ...
    const text = encodeURIComponent(message || config.whatsappDefaultMessage || "Olá!");
  ...
  function applyBrand() {
    ...
    if (document.title.includes("Nome da Empresa") && config.companyName) {
      document.title = document.title.replace("Nome da Empresa", config.companyName);
    }
  }

  function productCard(product) {
    const productName = escapeHTML(product.name);
    const message = `Olá! Vim pelo site e tenho interesse no produto "${product.name}". Gostaria de saber preço, prazo e opções disponíveis.`;

    return `
      <article class="product-card reveal visible">
        <div class="product-image">
          <img src="${escapeHTML(product.image)}" alt="${productName}" loading="lazy" />
          <span class="product-badge">${product.available === false ? "Indisponível" : "Sob demanda"}</span>
        </div>
        ...
        <div class="product-body">
          ...
          <div class="product-footer">
            <div class="product-price">
              <small>Valor</small>
              ${escapeHTML(product.price || "Sob consulta")}
            </div>
            <a
